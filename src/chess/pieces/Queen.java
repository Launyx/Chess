...
public class Queen extends ChessPiece{
    ...
    public boolean[][] possibleMoves() {    // Method to check the possible move for this piece
        ...
        // NW
        p.setValues(this.position.getRow() - 1, this.position.getColumn() -1);   // set new values for the position "p", in this case, to one row above

        // loop to check the matrix of the board
        while(getBoard().positionExists(p) && !getBoard().thereIsAPiece(p)){    // While the position "p" exists in the board AND there is no piece in that position
            mat[p.getRow()][p.getColumn()] = true;                              // Sets the position as true, making it a possible move to the Rook
            p.setValues(p.getRow() -1, p.getColumn() -1);                                          // sets a new row to the position "p", until it finds the end of the board or a piece
        }
        ...
        // NE
        p.setValues(this.position.getRow() -1, this.position.getColumn() +1);   // set new values for the position "p", in this case, to one column to the left

        // loop to check the matrix of the board
        while(getBoard().positionExists(p) && !getBoard().thereIsAPiece(p)){    // While the position "p" exists in the board AND there is no piece in that position
            mat[p.getRow()][p.getColumn()] = true;                              // Sets the position as true, making it a possible move to the Rook
            p.setValues(p.getRow() -1, p.getColumn() +1);                                     // sets a new column to the position "p", until it finds the end of the board or a piece
        }
        ...
        // SE
        p.setValues(this.position.getRow() +1, this.position.getColumn() +1);   // set new values for the position "p", in this case, to one column to the right
